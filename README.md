...
很简单吧？这还有些更有意思的：
	...
	- "Backstage passes"（后台通行证）与"Aged Brie"（陈年布利奶酪）类似，其品质`Quality`会   
	  随着时间推移而提高；当还剩10天或更少的时候，品质`Quality`每天提高2；当还剩5天或更少的      
      时候，品质`Quality`每天提高3；但一旦过期，品质就会降为0
...
#### 工程说明 
运行test目录下GildedRoseTextTests.cpp的main函数可以看到打印      
当添加测试时，把运行代码搬移到测试文件中后，要把main函数注释掉，不然运行会报错     
